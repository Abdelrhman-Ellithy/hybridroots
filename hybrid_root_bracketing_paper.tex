...
\begin{abstract}
Finding roots of nonlinear equations requires balancing the guaranteed convergence of traditional bracketing methods with the speed of optimization-based techniques. This paper introduces four novel hybrid algorithms: \textbf{Optimized Bisection-False Position (Optimized\_BF)}, \textbf{Optimized Bisection-False Position-Modified Secant (Optimized\_BFMS)}, \textbf{Optimized Trisection-False Position (Optimized\_TF)}, and \textbf{Optimized Trisection-False Position-Modified Secant (Optimized\_TFMS)} that synergistically combine interval-halving or trisection with accelerated interpolation steps. Performance is validated on a comprehensive benchmark of 42 diverse problems, including polynomial, transcendental, and ill-conditioned functions. We conduct a rigorous comparative analysis against traditional baselines (Bisection, False Position) and established state-of-the-art methods (Brent's method, Quadratic Interval Refinement). Our results show that the premier algorithm, \textbf{Optimized\_BFMS}, achieves the best overall performance, winning on \textbf{47.6\%} of problems and demonstrating significant speedups over both the fastest traditional baseline (Bisection, \textbf{5.3$\times$}) and state-of-the-art algorithms like Brent's method (\textbf{1.7$\times$}). Our findings establish these hybrid approaches as robust, efficient, and practical alternatives for numerical root finding.
\end{abstract}
...
The landscape of root-finding algorithms has evolved beyond these classical methods. Modern, state-of-the-art algorithms like Quadratic Interval Refinement (QIR) and the widely-deployed Brent's method are now standard benchmarks for high performance. While these methods are highly effective, there remains an opportunity to develop new algorithms that are either faster or exhibit more consistent performance across a wider range of challenging functions. This paper addresses that gap by introducing four new hybrid algorithms and evaluating them not only against traditional baselines but also against these established, high-performance algorithms.

The main contributions of this paper are:
\begin{enumerate}
    \item The design and implementation of \textbf{four novel hybrid algorithms} that blend reliable interval reduction (Bisection and Trisection) with accelerated interpolation and modified secant steps.
    \item A \textbf{comprehensive performance evaluation} on a diverse benchmark of 42 test functions, including high-degree polynomial, transcendental, and ill-conditioned problems.
    \item A \textbf{rigorous comparative analysis} against both traditional baselines and modern, state-of-the-art methods, namely QIR and two implementations of Brent's method.
    \item The empirical finding that our premier hybrid algorithm, \textbf{Optimized\_BFMS}, provides a superior balance of speed and reliability, demonstrably outperforming the established benchmarks on average and winning in a plurality of the test cases.
\end{enumerate}
